...
import io.micronaut.rabbitmq.connect.DefaultChannelPool;
...
public class RabbitMQConsumerAdvice implements ExecutableMethodProcessor<RabbitListener>, AutoCloseable {
    ...
    public void process(BeanDefinition<?> beanDefinition, ExecutableMethod<?, ?> method) {
        ...
        if (queueAnn != null) {
            ...
            List<Channel> channelList = getChannel(channelPool);
     ...
     */
    protected List<Channel> getChannel(ChannelPool channelPool) {
        try {
            return ((DefaultChannelPool) channelPool).getChannels();
        } catch (IOException e) {
